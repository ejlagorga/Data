...
def generate(data_fn, out_fn, N_epochs, solo, soffset, accomp, aoffset):
    # model settings
    max_len = 20
    max_tries = 1000
    diversity = 0.5
    ...
    # get data
    chords, abstract_grammars = get_musical_data(data_fn, solo, soffset, accomp, aoffset)
    ...
    for loopIndex in range(1, loopEnd):
        ...
        for n in curr_notes:
            note_score.insert(curr_offset + n.offset, n)
        for c in curr_chords:
            chord_score.insert(curr_offset + c.offset, c)

        curr_offset += 4.0

    #paste streams together
    out_stream.insert(0.0, tempo.MetronomeMark(number=bpm))

    # TODO: Change instruments
    note_score.insert(0, instrument.Saxophone())
    chord_score.insert(0, instrument.Piano())

    out_stream.insert(note_score)
    out_stream.insert(chord_score)
...
def main(args):
    ...
    fname = str(args[2])
    data_fn = fname
    out_fn =  fname[:fname.index('.')] + '_' + str(N_epochs)

    if (N_epochs == 1): out_fn += '_epoch.midi'
    else:               out_fn += '_epochs.midi'

    solo = int(args[3])
    soffset = list(map(int, args[4].strip('[]').split(',')))

    accomp = int(args[5])
    aoffset = list(map(int, args[6].strip('[]').split(',')))

    generate(data_fn, out_fn, N_epochs, solo, soffset, accomp, aoffset)

#    print("Usage: python3 generate.py <#epochs> <song.midi> <solo instrument> <accompaniment> <solo offset>")
#    print("Note: solo offset is [#,#] identifing midi offset for start and end of solo")
